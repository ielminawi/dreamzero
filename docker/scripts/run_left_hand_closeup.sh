...
#
# Usage:
#   sudo bash docker/scripts/run_left_hand_closeup.sh [extra args to snapshot_frame0_multiview.py]
# Examples:
#   sudo bash docker/scripts/run_left_hand_closeup.sh                 # frame 0
#   sudo bash docker/scripts/run_left_hand_closeup.sh --frame 250     # a later frame
set -uo pipefail
...
exec docker run --rm --gpus all \
  ...
  "$IMAGE" \
  -p eval_utils/render_left_hand_closeup.py \
