...
#
# Usage:
#   sudo bash docker/scripts/run_snapshot_frame0.sh [extra args to snapshot_frame0_multiview.py]
# Examples:
#   sudo bash docker/scripts/run_snapshot_frame0.sh                 # frame 0
#   sudo bash docker/scripts/run_snapshot_frame0.sh --frame 250     # a later frame
set -uo pipefail
...
exec docker run --rm --gpus all \
  ...
  "$IMAGE" \
  -p eval_utils/snapshot_frame0_multiview.py \
